...
public class PhaseShift implements CallableMessage.CallableMessageConsumer<Actor>, MessageNoRouting {
    ...
    public static class PhaseTerminalActor extends ActorDefault {
        ...
        public PhaseTerminalActor(ActorSystem system, Instant start, boolean closeSystem, BiConsumer<ActorSystem, PhaseCompleted> handler) {
            super(system);
            this.start = start;
    ...
    public static class PhaseEntry {
        ...
        public boolean processIntermediate(ActorAggregationReplicable self, PhaseShift.PhaseShiftIntermediate ps) {
            if (ps.getType().equals(PhaseShiftIntermediateType.PhaseIntermediateRouterStart)) {
                if (self.getMailboxAsReplicable().hasRemainingProcesses()) {
                    ...
                    Collection<ActorRef> canceled = ((ActorAggregationReplicable.StateSplitRouter) self.getState()).getCanceled();
        ...
        public boolean completedCancel(ActorAggregationReplicable router, ActorRef canceled) {
