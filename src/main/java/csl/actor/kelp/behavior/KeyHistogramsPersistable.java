...
import csl.actor.persist.PersistentFileManager.PersistentFileReaderSource;
 ...
 *       the method {@link PersistentConditionHistogram#needToPersist(String, HistogramTreePersistable, long, double)}
 ...
 *                <li>{@link PersistentConditionHistogramSampling}</li>
 *            </ul>
 *    </li>
 *
...
public class KeyHistogramsPersistable extends KeyHistograms {
    ...
    public static KeyHistograms createTreeFactory(boolean persist, boolean runtime, Mailbox mailbox, PersistentFileManager m, HistogramTreePersistableConfig config) {
        if (persist) {
            return new KeyHistogramsPersistable(config, m,
                    createCondition(runtime, mailbox, m, config));
        } else {
            return new KeyHistograms(m);
        }
    }

    public static PersistentConditionHistogram createCondition(boolean runtime, Mailbox mailbox, PersistentFileManager m, HistogramTreePersistableConfig config) {
        if (runtime) {
            if (mailbox instanceof MailboxManageable) {
                return new PersistentConditionHistogramSampling(
                        config.getHistogramPersistSizeLimit(), config.getHistogramPersistSizeRatioThreshold(),
                        (MailboxManageable) mailbox, m.getLogger());
            } else {
                return new PersistentConditionHistogramSampling(
                        config.getHistogramPersistSizeLimit(), config.getHistogramPersistSizeRatioThreshold(),
                        m.getLogger());
            ...
            return new PersistentConditionHistogramSizeLimit(config);
        }
    }
    ...
    public static class PersistentConditionHistogramSampling implements PersistentConditionHistogram {
        ...
        protected PersistentConditionMailbox.PersistentConditionMailboxSampling mailboxSampling;
        protected long sizeLimit;
        protected double sizeRatioThreshold;
        protected PersistentConditionMailbox.SampleTiming logTimingNeedToPersist = new PersistentConditionMailbox.SampleTiming();
        ...
        public PersistentConditionHistogramSampling(long sizeLimit, double sizeRationThreshold, MailboxManageable mailbox,
                                                    ActorSystem.SystemLogger logger) {
            this.sizeLimit = sizeLimit;
            this.sizeRatioThreshold = sizeRationThreshold;
            this.mailbox = mailbox;
            if (mailbox != null &&
                    mailbox.getCondition() instanceof PersistentConditionMailbox.PersistentConditionMailboxSampling) {
                mailboxSampling = (PersistentConditionMailbox.PersistentConditionMailboxSampling) mailbox.getCondition();
            } else {
                mailboxSampling = new PersistentConditionMailbox.PersistentConditionMailboxSampling(sizeLimit, logger);
            }
        }
        ...
        public HistogramPersistentOperation needToPersist(String callerInfo, HistogramTreePersistable tree,
            ...
            if (onMemoryValues > sizeLimit) {
                ...
                if (log || res) {
                    mailboxSampling.getLogger().log(PersistentFileManager.logPersist, logPersistColor,
    ...
    public static class HistogramTreePersistable extends HistogramTree implements HistogramTreePersistableConfig {
        ...
        public HistogramTreePersistable(HistogramNode root, KeyComparator<?> comparator, int treeLimit,
                                        HistogramTreePersistableConfig config,
                                        PersistentFileManager persistent,
                                        PersistentConditionHistogram condition) {
        ...
        public void init(PersistentFileManager persistent, PersistentConditionHistogram condition) {
            init(persistent);
        ...
        @Override
        public int getHistogramPersistHistoryEntrySize() { return this.historyEntrySize; }
        @Override
        public int getHistogramPersistHistoryEntryLimit() { return this.historyEntryLimit; }
        @Override
        public long getHistogramPersistSizeLimit() { return this.sizeLimit; }
        @Override
        public long getHistogramPersistOnMemorySize() { return this.onMemorySize; }
        @Override
        public double getHistogramPersistSizeRatioThreshold() { return this.sizeRatioThreshold; }
        @Override
        public long getHistogramPersistRandomSeed() { return this.randomSeed; }
        ...
        public boolean needToReduce() {
            HistogramPersistentOperation op = checkPersist("needToReduce", getTreeSizeOnMemory() + 10L);
            return (!op.equals(HistogramPersistentOperationType.None));
        }
        ...
        public void persistLargeLeaves(HistogramNode node, PersistentFileManager.PersistentFileWriter w, long keepSize) {
        ...
        public void persistList(List<HistogramLeafList> valuesList, PersistentFileManager.PersistentFileWriter w, long keepSize) {
        ...
        protected void logPersistedTreeBefore(long persistingLimit, Object obj) {
            PersistentFileManager m = getPersistent();
            if (m != null) {
                m.getLogger().log(PersistentFileManager.logPersist, logPersistColor,
    ...
    public static class TreeWriting implements Cloneable, AutoCloseable {
        ...
        public TreeWriting(PersistentFileManager manager, String pathExpanded) throws IOException {
        ...
        public Object lookAhead(HistogramTree tree) {
            try {
                ...
                if (v instanceof HistogramLeafCellSerializedEnd) {
                    reader.getManager().getLogger().log(PersistentFileManager.logDebugPersist,
    ...
    public static class PersistentFileReaderSourceWithSize implements Serializable {
        ...
        public PersistentFileManager.PersistentFileReader createReader() {
            ...
            if (!loaded) {
                ...
                try (PersistentFileManager.PersistentFileReader r = source.createReader()) {
                    ...
                    while (true) {
                        ...
                        if (childObj instanceof PersistentFileManager.PersistentFileEnd) {
                            break;
                    ...
                    this.children = cs;
                    updateChildren();
                    r.getManager().getLogger().log(PersistentFileManager.logDebugPersist, logPersistColor, "close: %s", r);
                } catch (Exception ex) {
