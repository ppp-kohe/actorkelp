...
import csl.actor.kelp_old.KelpRoutingSplit.SplitPath;
...
public abstract class ActorKelp<SelfType extends ActorKelp<SelfType>> extends ActorDefault
    ...
    protected MailboxDefault initMailboxDefault(PersistentFileManager m) {
        if (initPersistentEnabled()) {
            PersistentConditionMailbox condMailbox;
            if (persistRuntimeCondition()) {
                condMailbox = new PersistentConditionMailbox.PersistentConditionMailboxSampling(persistMailboxSizeLimit(), m.getLogger());
            } else {
                condMailbox = new PersistentConditionMailbox.PersistentConditionMailboxSizeLimit(persistMailboxSizeLimit());
            }
            return new MailboxPersistableReplacement(m, condMailbox, persistMailboxOnMemorySize());
        } else {
    ...
    protected KeyHistograms initTreeFactory(MailboxDefault mailbox, PersistentFileManager m) {
        if (initPersistentEnabled()) {
            ...
            if (persistRuntimeCondition()) {
                if (mailbox instanceof MailboxPersistableReplacement) {
                    condHist = new KeyHistogramsPersistable.PersistentConditionHistogramSampling(
                            histogramPersistSizeLimit(), histogramPersistSizeRatioThreshold(), (MailboxPersistableReplacement) mailbox, m.getLogger());
                } else {
                    condHist = new KeyHistogramsPersistable.PersistentConditionHistogramSampling(
